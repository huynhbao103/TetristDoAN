...
public class Board2 extends JPanel implements KeyListener, MouseListener, MouseMotionListener {
	...
	// field
    private Color[][] board2 = new Color[boardHeight][boardWidth];

	// array with all the possible shapes
    private Shape2[] shapes = new Shape2[7];

	// currentShape
    private static Shape2 currentShape, nextShape;
    ...
    public Board2() {
		...
		// create shapes
        shapes[0] = new Shape2(new int[][]{
            {1, 1, 1, 1} // I shape;
        }, this, colors[0]);

        shapes[1] = new Shape2(new int[][]{
            {1, 1, 1},
            {0, 1, 0}, // T shape;
        }, this, colors[1]);

        shapes[2] = new Shape2(new int[][]{
            {1, 1, 1},
            {1, 0, 0}, // L shape;
        }, this, colors[2]);

        shapes[3] = new Shape2(new int[][]{
            {1, 1, 1},
            {0, 0, 1}, // J shape;
        }, this, colors[3]);

        shapes[4] = new Shape2(new int[][]{
            {0, 1, 1},
            {1, 1, 0}, // S shape;
        }, this, colors[4]);

        shapes[5] = new Shape2(new int[][]{
            {1, 1, 0},
            {0, 1, 1}, // Z shape;
        }, this, colors[5]);

        shapes[6] = new Shape2(new int[][]{
            {1, 1},
            {1, 1}, // O shape;
        }, this, colors[6]);
...
public void addScore(int linesCleared) {
        ...
        for (int row = 0; row < nextShape.getCoords().length; row++) {
            for (int col = 0; col < nextShape.getCoords()[0].length; col++) {
                if (nextShape.getCoords()[row][col] != 0) {
                    g.fillRect(col * 30 + 320, row * 30 + 50, Board2.blockSize, Board2.blockSize);
                }
            }
        }
    ...
    checkLine();
        ...
        if (gamePaused) {
            ...
            g.drawString(gamePausedString, 35, Tetrist2.HEIGHT / 2);
        }
        if (gameOver) {
            String gameOverString = "GAME OVER";
            g.setColor(Color.WHITE);
            g.setFont(new Font("Georgia", Font.BOLD, 30));
            g.drawString(gameOverString, 50, Tetrist2.HEIGHT / 2);
        }
        g.setColor(Color.WHITE);

        g.setFont(new Font("Georgia", Font.BOLD, 20));

        g.drawString("SCORE", Tetrist2.WIDTH - 555, Tetrist2.HEIGHT / 2);
        g.drawString(score + "", Tetrist2.WIDTH - 555, Tetrist2.HEIGHT / 2 + 30);
    ...
    public void setNextShape() {
        ...
        nextShape = new Shape2(shapes[index].getCoords(), this, colors[colorIndex]);
    }
        ...
        for (int row = 0; row < currentShape.getCoords().length; row++) {
            for (int col = 0; col < currentShape.getCoords()[0].length; col++) {
                if (currentShape.getCoords()[row][col] != 0) {
                    if (board2[currentShape.getY() + row][currentShape.getX() + col] != null) {
                        gameOver = true;
                    }
                }
            }
